...
export default function Home({ session }) {
  ...
  // Intersection Observer 훅 추가
  const FadeInComponent = ({ children, delay = 0 }) => {
    const [isVisible, setIsVisible] = useState(false);
    const domRef = import('react').then(React => React.useRef()); // Or just use a simpler approach

    // Since we can't easily use hooks conditionally or inside a generic component without importing,
    // let's just do a simple CSS animation class or use a basic IntersectionObserver hook.
    return null;
  };
    ...
    return (
      <div className="bg-[#121214] min-h-screen text-white font-['Inter'] relative overflow-x-hidden leading-[1.5]">
        ...
        <section className="py-24 md:py-32 px-6 text-center border-t border-gray-800/50 bg-gradient-to-b from-transparent to-accent-blue/10">
          <h2 className="text-3xl md:text-4xl font-bold mb-8 break-keep">3초 만에 가입하고 모의 입찰 시작하기</h2>
